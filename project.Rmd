...
confusionMatrix(predict1, myTesting$classe)$overall[1] #accuracy of 0.7295
```

##Random Forest
```{r}
fit2 <- randomForest(classe ~. , data=myTraining)
predict2 <- predict(fitt2, myTesting, type = "class")
confusionMatrix(predict2, myTesting$classe)$overall[1] #accuracy of 0.8193
```


#Conclussion
The Random Forest model was the best one of all tested models, as it was expected, and it is the recomended model for future predictions for this specific dataset.
